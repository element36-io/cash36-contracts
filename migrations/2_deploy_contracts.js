...
module.exports = function (deployer, network, accounts) {
    ...
    if (network === 'local' || network === 'dev') {
        deployer.deploy(EthereumClaimsRegistry).then(function() {
            return deployer.deploy(Cash36KYC, EthereumClaimsRegistry.address, accounts[2]).then(function() {
                    return deployer.deploy(Cash36, Cash36KYC.address);
                });
        });
    } else {
