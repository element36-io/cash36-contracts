/**
 * Contains all Test regarding GBP36
 */
...
const Token36Controller = artifacts.require("./GBP36Controller.sol");
...
contract('Create and Test GBP36', function (accounts) {

    var Cash36Instance;
    var Cash36ComplianceInstance;
    var GBP36Instance;
    var GBP36ControllerInstance;
    ...
    before("...get Cash36Instance.", async function () {
        ...
        var tokenAddress = await Cash36Instance.getTokenBySymbol('GBP36');
        GBP36Instance = await Token36.at(tokenAddress);

        var tokenControllerAddress = await GBP36Instance.controller();
        GBP36ControllerInstance = await Token36Controller.at(tokenControllerAddress);
  ...
  it("...it should allow to administrate cash36 contracts.", async function () {
    var maxAccountTokensBefore = await Cash36Instance.getMaxAccountTokens('GBP36');
    //assert.equal(maxAccountTokensBefore, "-1", "The maxAccountTokensBefore was not correct.");

    await Cash36Instance.setMaxAccountTokens('GBP36', format(10000));

    var maxAccountTokensAfter = await Cash36Instance.getMaxAccountTokens('GBP36');
    assert.equal(maxAccountTokensAfter, format(10000), "The maxAccountTokensAfter was not correct.");

    var compliance = await Cash36Instance.getCompliance('GBP36');
    assert.equal(compliance, Cash36Compliance.address, "The compliance address was not correct.");
  });

    it("...it should mint 100 GBP36 and assign it to accounts[1].", async function () {
        ...
        await GBP36ControllerInstance.mint(accounts[ 1 ], format(100), { from: accounts[ 0 ] });

        var newBalanceFor1 = await GBP36Instance.balanceOf(accounts[ 1 ]);
        assert.equal(parse(newBalanceFor1), "100", "The balance was not correct.");

        var totalSupply = await GBP36Instance.totalSupply();
    ...
    it("...it should not allow minting from another account.", async function () {
        ...
        //await expectThrow(GBP36ControllerInstance.mint(accounts[ 1 ], 100, { from: accounts[ 1 ] }));
    });

    it("...it should not allow to burn 50 GBP36.", async function () {
        // TODO: figure out how to test throws in JS, otherwise write Sol Tests
        //await expectThrow(GBP36ControllerInstance.burn(accounts[ 1 ], 50, { from: accounts[ 0 ] }));
    });

    it("...it should burn 50 GBP36 and remove it from accounts[1].", async function () {
        //await GBP36Instance.approve(GBP36ControllerInstance.address, 50, { form: accounts[ 1 ] });
        await GBP36Instance.burn(format(50), { from: accounts[ 1 ] });

        var newBalanceForFee = await GBP36Instance.balanceOf(accounts[ 0 ]);
    ...
    it("...it should not allow burning from another account.", async function () {
        // TODO: figure out how to test throws in JS, otherwise write Sol Tests
        //await expectThrow(GBP36ControllerInstance.burn(accounts[ 1 ], 100, { from: accounts[ 1 ] }));
    });

    it("...it should not allow to transfer 25 GBP36 to accounts[2] not being KYCed.", async function () {
        // TODO: figure out how to test throws in JS, otherwise write Sol Tests
        //await GBP36Instance.transfer(accounts[ 2 ], 25, { from: accounts[ 1 ] });
    });

    it("...it should allow to transfer 25 GBP36 to accounts[2].", async function () {
        ...
        await GBP36Instance.transfer(accounts[ 2 ], format(25), { from: accounts[ 1 ] });

        var newBalanceFor1 = await GBP36Instance.balanceOf(accounts[ 1 ]);
        assert.equal(parse(newBalanceFor1), "25", "The balance was not correct.");

        var newBalanceFor2 = await GBP36Instance.balanceOf(accounts[ 2 ]);
    ...
    it("...it should allow to transferFrom 5 GBP36 to accounts[2] as accounts[3].", async function () {
        var allowance = await GBP36Instance.allowance(accounts[ 1 ], accounts[ 3 ]);
        assert.equal(allowance, "0", "The allowance was not correct.");

        await GBP36Instance.approve(accounts[ 3 ], format(5), { from: accounts[ 1 ] })

        allowance = await GBP36Instance.allowance(accounts[ 1 ], accounts[ 3 ]);
        assert.equal(parse(allowance), "5", "The allowance was not correct.");

        await GBP36Instance.transferFrom(accounts[ 1 ], accounts[ 2 ], format(5), { from: accounts[ 3 ] });
    ...
    it("...it should not allow to transfer if accounts[2] is on blacklist.", async function () {
        ...
        // TODO: figure out how to test throws in JS, otherwise write Sol Tests
        //await GBP36Instance.transfer(accounts[ 1 ], 5, { from: accounts[ 2 ] });
        //assert.equal(result, false, "The result of transfer was not correct.");

        var newBalanceFor2 = await GBP36Instance.balanceOf(accounts[ 1 ]);
        ...
        await GBP36Instance.transfer(accounts[ 1 ], format(5), { from: accounts[ 2 ] });

        var newBalanceFor2After = await GBP36Instance.balanceOf(accounts[ 1 ]);
    ...
    it("...it should not allow to transfer if token is disabled.", async function () {
        var enabled = await GBP36Instance.transfersEnabled();
        assert.equal(enabled, true, "The token was not enabled.");

        await Cash36Instance.enableTransfers('GBP36', false, { from: accounts[ 0 ] });
